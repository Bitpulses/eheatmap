...
def _draw_sub_heatmap(ax, sub_df, cmap_obj, norm, border_color, use_border,
                      ...
                      has_numbers, is_split=False, split_border_color=None, split_border_width=None):
    """Draw a single heatmap sub-block (used for split rendering)."""
    
    # 1. Draw heatmap mesh
    # Use border_color for cell borders (grid lines inside the block)
    ax.pcolormesh(
        sub_df.values,
        cmap=cmap_obj,
        norm=norm,
    ...
    # 3. Handle borders (spines)
    # If splitting, use axes spines to draw the border around the block.
    # This ensures the border is perfectly aligned with the block.
    if is_split and split_border_color:
        ...
        # Otherwise hide spines
        for spine in ax.spines.values():
            spine.set_visible(False)

    # 4. Add numbers if requested
    if has_numbers:
...
def pheatmap(
    ...
    if n_row_groups > 1 or n_col_groups > 1:
        ...
        # Calculate exact wspace and hspace to achieve consistent visual gaps
        # regardless of the heatmap's aspect ratio.
        #
        # GridSpec wspace is defined as: gap_width = wspace * avg_cell_width.
        # We have: N_cells * cell_width + (N_cells - 1) * gap_width = Total_Width.
        # We want gap_width to be exactly target_px.
        # Therefore: cell_width = (Total_Width - (N_cells - 1) * target_px) / N_cells
        # And: wspace = target_px / cell_width
        # => wspace = (N_cells * target_px) / (Total_Width - (N_cells - 1) * target_px)

        # Convert split gaps from mm to pixels
        # We also add compensation for the border width to maintain the visual gap size.
        # split_border_width is in points (1/72 inch)
        border_comp_px = split_border_width * dpi / 72.0
        
        # Row split gap -> Vertical gap -> hspace
        if n_row_split > 1:
            target_row_gap_px = row_split_gap * mm2inch * dpi + border_comp_px
            n = n_row_split
        ...
        # Column split gap -> Horizontal gap -> wspace
        if n_col_split > 1:
            target_col_gap_px = col_split_gap * mm2inch * dpi + border_comp_px
            n = n_col_split
    ...
    if n_row_groups > 1 or n_col_groups > 1:
        ...
        for i in range(n_row_split):
            for j in range(n_col_split):
                ...
                if j == 0 and show_rownames:
                    ax.set_yticks(np.arange(sub.shape[0]) + 0.5)
                    ax.set_yticklabels(sub.index, fontsize=fontsize_row, ha="right")
                    ax.tick_params(axis="y", length=0)

                # Column labels: only on the bottom row
                if i == n_row_split - 1 and show_colnames:
        ...
        if show_rownames:
            ...
            main_ax.set_yticklabels(df.index, fontsize=fontsize_row, ha="right")
    ...
    if row_Z is not None and "dendro_row" in ci:
        if n_row_split > 1:
            ...
            # Set linewidth for all lines and collections in dendrogram
